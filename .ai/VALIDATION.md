...
- Physical and `vm-dev` entry points → **Not validated** end-to-end (require a desktop with snap/flatpak; out of scope for a plain container). Function-level run of every apt-based installer those entry points use (base, git, java, python, Docker, kubectl, AWS CLI, Chrome, VS Code, Sublime, Terminator, zsh, Flameshot, Remmina, X2Go, Flatpak) on `ubuntu:24.04` arm64 → **Validated** (all pass). Spotify is `amd64`-only (not found on arm64; guard added) and AnyDesk downloaded/installed but its postinst failed only because the container has no systemd (treated as an environment artifact).
- Snap-based installs (`teams-for-linux`, `kontena-lens`) and `install_teamviewer`/`install_virtualbox`/`install_discord` → **Not validated** in the container (snapd needs systemd; TeamViewer/VirtualBox need `amd64` and a real kernel). Vendor URLs and keys were reachability-checked: Spotify/AnyDesk/Sublime/k8s keys return 200, and TeamViewer `amd64`/`arm64` debs return 200.
