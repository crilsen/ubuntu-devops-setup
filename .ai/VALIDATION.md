...
- `ubuntu-system-prepare-vm-dev.sh` end-to-end in the same `ubuntu:24.04` arm64 container (`APT_UPGRADE=0`) → **Validated**, exit 0 (before AnyDesk/TeamViewer were removed in ADR-009). Every install step passed on arm64, including Flatpak+Flathub, Chrome, VS Code, Sublime, Terminator, Flameshot, Remmina, and **X2Go**. The VM target has no `amd64`-only package, so it runs unmodified on arm64.
...
- Snap-based installs (`teams-for-linux`, `kontena-lens`), `install_discord`, and `install_virtualbox` → **Not validated** in the container (snapd needs systemd; VirtualBox is `amd64`-only and needs a real kernel). Vendor keys were reachability-checked: Spotify, Sublime, and `pkgs.k8s.io` return 200.
